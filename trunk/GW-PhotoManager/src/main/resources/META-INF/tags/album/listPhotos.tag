...
<%@ attribute name="objects" required="true" rtexprvalue="true" type="java.util.List" %>
...
--%>
<c:forEach var="foto" items="${objects}">
	<div class="${lineClass}" style="float: left">
		...
		<c:if test="${showName}">
			<div class="${nameClass}"><c:out value="${foto.nome}" /></div>
		</c:if>
		<c:if test="${showLocation}">
			<div class="${locationClass}"><c:out value="${foto.lugar}" />
			</div>
		</c:if>
	</div>
</c:forEach>